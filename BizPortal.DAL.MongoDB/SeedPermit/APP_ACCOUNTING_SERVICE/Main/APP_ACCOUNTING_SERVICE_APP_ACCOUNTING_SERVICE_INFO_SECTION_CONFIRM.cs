...
{
    public partial class APP_ACCOUNTING_SERVICE_APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM
    {
        public static void Init()
        {
            ...
            if (db.AsQueryable().Where(x => x.Section == "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM").Count() == 0)
            {
                items.Add(new FormSection()
                {
                    Section = "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM",
            ...
            if (db.AsQueryable().Where(x => x.Section == "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM").Count() == 0)
            {
                items.Add(new FormSectionRow()
                {
                    ...
                    Controls = new List<FormControl>()
                    {
                        ...
                        {
                            FieldID = "F43_01_31",
                            Control = "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM_CONFIRM",
                            Type = ControlType.CheckBox,
                            DataKey = "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM_CONFIRM",
                            Info = "APP_ACCOUNTING_SERVICE_INFO_SECTION_CONFIRM_CONFIRM_INFO",
