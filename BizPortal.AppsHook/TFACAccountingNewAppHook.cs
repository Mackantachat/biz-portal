...
{
    public class TFACAccountingNewAppHook : SingleFormRendererAppHook
    {
        ...
        public override bool InvokeSingleForm(Guid trid, string currentSectionGroup, ref SingleFormRequestViewModel model)
        {
            ...
            else if (currentSectionGroup == "APP_ACCOUNTING_SERVICE")
            {
                if (model.IdentityType == UserTypeEnum.Juristic)
                {
                    var committeeLicenseData = GetSectionData(model, "APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE", SectionType.Form);
                    var committeeData = GetSectionData(model, "COMMITTEE_INFORMATION", SectionType.ArrayOfForms);

                    var data = new SingleFormSectionDataViewModel
                    {
                        SectionName = "APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE",
                        Type = SectionType.Form.ToString()
                    };

                    // Remove existing committee license data
                    model.SectionData.Remove(committeeLicenseData);

                    string strLicenseData = committeeLicenseData.FormData.TryGetString("APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE_LICENSE", "[]");
                    JArray jLicenseArr = JsonConvert.DeserializeObject<JArray>(strLicenseData);

                    List<object> commList = new List<object>();
                    foreach (var c in committeeData.ArrayData)
                    {
                        if (c.TryGetString("JURISTIC_COMMITTEE_IS_AUTHORIZED_OPTION", "").ToUpper() == "YES")
                        {
                            string existingLicense = "";
                            foreach (JObject jLicense in jLicenseArr)
                            {
                                if (jLicense.ContainsKey("TITLE") && jLicense["TITLE"].ToString() == c.TryGetString("JURISTIC_COMMITTEE_TITLE")
                                    && jLicense.ContainsKey("NAME") && jLicense["NAME"].ToString() == c.TryGetString("JURISTIC_COMMITTEE_NAME")
                                    && jLicense.ContainsKey("LASTNAME") && jLicense["LASTNAME"].ToString() == c.TryGetString("JURISTIC_COMMITTEE_LASTNAME"))
                                {
                                    existingLicense = jLicense.ContainsKey("LICENSE") ? jLicense["LICENSE"].ToString() : "";
                                }
                            }

                            commList.Add(new
                            {
                                TITLE = c.TryGetString("JURISTIC_COMMITTEE_TITLE"),
                                NAME = c.TryGetString("JURISTIC_COMMITTEE_NAME"),
                                LASTNAME = c.TryGetString("JURISTIC_COMMITTEE_LASTNAME"),
                                LICENSE = existingLicense
                            });
                        }
                    }

                    data.FormData.Add("APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE_LICENSE", JsonConvert.SerializeObject(commList));

                    model.SectionData.Add(data);
                }
                ...
                switch (stage)
                {
                    ...
                    case AppsStage.UserUpdate:
                        if (model.Status == ApplicationStatusV2Enum.CHECK || model.Status == ApplicationStatusV2Enum.PENDING)
                        {
                            ...
                            else if (request_date_type == "OTHER_DATE")
                            {
                                request_date = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_INFO_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_INFO_SECTION_OTHER_DATE_OPTION");
                            }
                            ...
                            var post = new TFACDataService()
                            {
                                data = new Data()
                                {
                                    ...
                                    Request_Form_ID = "5", // 5 คือ ขอจดทะเบียน
                                    ...
                                    Capital = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_INFO_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_INFO_SECTION_REGISTER_CAPITAL").Replace(",", ""),
                                    Fiscal_Year_End_Date = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetDateStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_POLICY_DATE"),
                                    ...
                                    Request_Date = DateTime.Now.ToString("yyyy-MM-dd", CultureInfo.GetCultureInfo("en-US")), // วันที่ยืนคำขอ
                                    ...
                                    Request_Date_Type_Date = TFACUtility.GetDateFormat(request_date),
                                    ...
                                    Is_Audited_Financial_Report = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_STATEMENT_OPTION") == "BUDGET" ? "1" : "2",// value -> 1= BUDGET/2 = FORECAST
                                                                                                                                                                                                                                         // model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetIntData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_CUSTOMER_AMOUNT"),
                                    Accounting_Customer_Amount = int.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_CUSTOMER_AMOUNT").Replace(",", ""), out amount) ? amount : 0,
                                    Accounting_Customer_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_CUSTOMER_INCOME"), out number) ? number : 0,
                                    Accounting_Customer_None_Amount = int.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_CUSTOMER_NONE_AMOUNT").Replace(",", ""), out amount) ? amount : 0,
                                    Accounting_Customer_None_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_CUSTOMER_NONE_INCOME"), out number) ? number : 0,
                                    Accounting_Total_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_ACCOUNTING_TOTAL_INCOME"), out number) ? number : 0,
                                    //model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetIntData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_AMOUNT"),
                                    Auditoring_Customer_Amount = int.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_AMOUNT").Replace(",", ""), out amount) ? amount : 0,
                                    Auditoring_Customer_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_INCOME"), out number) ? number : 0,
                                    //model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetIntData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_NONE_AMOUNT")
                                    Auditoring_Customer_None_Amount = int.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_NONE_AMOUNT").Replace(",", ""), out amount) ? amount : 0,
                                    Auditoring_Customer_None_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_CUSTOMER_NONE_INCOME"), out number) ? number : 0,
                                    Auditoring_Total_Income = Decimal.TryParse(model.Data.TryGetData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_CALCULATE_SECTION_AUDITORING_TOTAL_INCOME"), out number) ? number : 0,
                                }
                            };
                            #region[Address]
                            ...
                            if (committeeTotal > 0)
                            {
                                ...
                                for (int i = 0; i < committeeTotal; i++)
                                {
                                    var committee = new Person
                                    {
                                        ...
                                        ID_No = model.Data.TryGetData("COMMITTEE_INFORMATION").ThenGetStringData("COMMITTEE_INFORMATION_CITIZEN_ID_" + i).Replace("-", ""),
                                        CPA_No = "",//model.Data.TryGetData("COMMITTEE_INFORMATION").ThenGetStringData("JURISTIC_COMMITTEE_ACCOUNTING_LICENSE_ID_" + i).Replace("-", ""),
                                        Accounting_No = "",//ไม่มี
                                    ...
                                    if (post.data.Corporate_Service_Type_ID != "1")
                                    {


                                        if (committee.Is_Authorize == 1)
                                        {
                                            string strLicenseData = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE").ThenGetStringData("APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE_LICENSE");
                                            //string strLicenseData = GetSectionData(model, "", SectionType.Form).FormData.TryGetString("APP_ACCOUNTING_SERVICE_INFO_SECTION_COMMITTEE_LICENSE", "[]");
                                            JArray jLicenseArr = JsonConvert.DeserializeObject<JArray>(strLicenseData);
                                            //string existingLicense = "";
                                            foreach (JObject jLicense in jLicenseArr)
                                            {
                                                if (jLicense.ContainsKey("TITLE") && jLicense["TITLE"].ToString() == committee.Title_TH
                                                    && jLicense.ContainsKey("NAME") && jLicense["NAME"].ToString() == committee.First_Name_TH
                                                    && jLicense.ContainsKey("LASTNAME") && jLicense["LASTNAME"].ToString() == committee.Last_Name_TH)
                                                {
                                                    committee.CPA_No = jLicense.ContainsKey("LICENSE") ? jLicense["LICENSE"].ToString() : "";
                                                }
                                            }
                                        }
                                    }
                            ...
                            //var managerTotal = int.Parse(model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_TOTAL"));
                            //if (managerTotal > 0)
                            //{
                            ...
                            //            ID_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_ID_" + i),
                            //            CPA_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_AUDITOR_ID_" + i),
                            //            Accounting_No = "",//ไม่มี
                            //            Title_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("DROPDOWN_APP_ACCOUNTING_SERVICE_MANAGER_SECTION_TITLE_TEXT_" + i),
                            //            First_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FIRSTNAME_" + i),
                            //            Last_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_LASTNAME_" + i),                 
                            //            Is_FullTime = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_WORKING_TYPE_OPTION_" + i) == "FULLTIME" ? 1 : 0,
                            ...
                            {
                                Person_Type_ID = (int)PersonType.Manager,
                                ID_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_ID").Replace("-", ""),
                                CPA_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_AUDITOR_ID"),
                                Accounting_No = "",//ไม่มี
                                Title_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("DROPDOWN_APP_ACCOUNTING_SERVICE_MANAGER_SECTION_TITLE_TEXT"),
                                First_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FIRSTNAME"),
                                Last_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_LASTNAME"),
                                Is_FullTime = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_FORM").ThenGetStringData("APP_ACCOUNTING_SERVICE_MANAGER_SECTION_WORKING_TYPE_OPTION") == "FULLTIME" ? 1 : 0,
                                Is_Authorize = 0, // 0 because manager section did not have this value
                            };
                            managerList.Add(manager);
                            PersonList.AddRange(managerList);

                            var accountantTotal = int.Parse(model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_TOTAL"));
                            if (accountantTotal > 0)
                            {
                                ...
                                for (int i = 0; i < accountantTotal; i++)
                                {
                                    var accountant = new Person
                                    {
                                        ...
                                        ID_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_ID_" + i).Replace("-", ""),
                                        CPA_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_AUDITOR_ID_" + i),
                                        Accounting_No = "", //ไม่มี
                                        Title_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("DROPDOWN_APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_TITLE_TEXT_" + i),
                                        First_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_FIRSTNAME_" + i),
                                        Last_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_LASTNAME_" + i),
                                        Is_FullTime = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_ACCOUTANT_SECTION_WORKING_TYPE_OPTION_" + i) == "FULLTIME" ? 1 : 0,
                            ...
                            var auditorTotal = int.Parse(model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_TOTAL"));
                            if (auditorTotal > 0)
                            {
                                ...
                                for (int i = 0; i < auditorTotal; i++)
                                {
                                    var auditor = new Person
                                    {
                                        ...
                                        ID_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_ID_" + i).Replace("-", ""),
                                        CPA_No = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_AUDITOR_ID_" + i),
                                        Accounting_No = "",//ไม่มี
                                        Title_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("DROPDOWN_APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_TITLE_TEXT_" + i),
                                        First_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_FIRSTNAME_" + i),
                                        Last_Name_TH = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_LASTNAME_" + i),
                                        Is_FullTime = model.Data.TryGetData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION").ThenGetStringData("APP_ACCOUNTING_SERVICE_AUDITOR_SECTION_WORKING_TYPE_OPTION_" + i) == "FULLTIME" ? 1 : 0,
                            ...
                            // พันธบัตรรัฐบาลไทย
                            var THAI_BONDS = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION_TYPE_OPTION");
                            if (THAI_BONDS == "THAI_BONDS")
                            {
                            ...
                            // บัญชีเงินฝากประจำ
                            var DEPOSIT = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION_TYPE_OPTION");
                            if (DEPOSIT == "DEPOSIT")
                            {
                                ...
                                for (int i = 0; i < DEPOSIT_Total; i++)
                                {
                                    var quarantee = new Guarantee
                                    {
                                        ...
                                        Month_Number = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_DEPOSIT_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_DEPOSIT_SECTION_DEPOSIT_TIME_MONTH_" + i),
                            ...
                            // บัตรเงินฝาก
                            var DEPOSIT_CARD = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION_TYPE_OPTION");
                            if (DEPOSIT_CARD == "DEPOSIT_CARD")
                            {
                            ...
                            var CORPARATE_BONDS = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION_TYPE_OPTION");
                            if (CORPARATE_BONDS == "CORPARATE_BONDS")
                            {
                            ...
                            // กรมธรรม์ประกันภัย
                            var POLICY = model.Data.TryGetData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION").ThenGetStringData("APP_ACCOUNTING_DETAIL_PROVIDING_SECTION_TYPE_OPTION");
                            if (POLICY == "POLICY")
                            {
                            ...
                            foreach (FileGroup group in model.UploadedFiles)
                            {
                                foreach (var item in group.Files)
                                {
                                    ...
                                    // if(count < 10) attachList.Add(attach);
                            ...
                            if (apiResult != null)
                            {
                                if (apiResult.HasValues)
                                {
                                    ...
                                    {
                                        result.Success = true;
                                        result.Data = GenerateAppsHookData(AppsHookResult.Created, stage, "", DataResult.ToString(), jsonPost);
                                    }

                                    if (DataResult.HasValues && DataResult["IsSuccess"].ToString() == "False")
                                    {
                                        result.Success = false;
                                        //result.Message = DataResult["Msg"].ToString();
                                        result.Data = GenerateAppsHookData(AppsHookResult.Failed, stage, DataResult["Msg"].ToString(), null, jsonPost, true);
                                    }
                                }
                                else
                                {
