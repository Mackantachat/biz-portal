...
{
    public class BKKTAXBoardCancelAppHook : StoreBaseAppHook
    {
        ...
        public override byte[] GetOrgPdfFormContent(ApplicationRequestEntity req, Func<string, string> serverMapPathFunc)
        {
            ...
            #region Header
            var generalInfo = req.Data["GENERAL_INFORMATION"];

            int taxYear = 0;
            if (int.TryParse(generalInfo.Data.TryGetString("APP_TAX_YEAR", ""), out taxYear))
            {
                if (taxYear < 2300) taxYear += 543; // Convert to Bc.
                model.Add(new PDFFieldValue() { FieldName = "Year", Value = (taxYear % 100).ToString() });
            }

            model.Add(new PDFFieldValue() { FieldName = "LastReceiveNo", Value = generalInfo.Data.TryGetString("APP_TAX_OLD_NUMBER") });

            if (req.IdentityType == UserTypeEnum.Citizen)
            {
                field = new PDFFieldValue() { FieldName = "IdentityName" };
                field.Value = string.Format("{0}{1} {2}", generalInfo.Data["DROPDOWN_CITIZEN_TITLE_TEXT"], generalInfo.Data["CITIZEN_NAME"], generalInfo.Data["CITIZEN_LASTNAME"]);
                model.Add(field);
            }
            else if (req.IdentityType == UserTypeEnum.Juristic)
            {
                field = new PDFFieldValue() { FieldName = "IdentityName" };
                field.Value = generalInfo.Data["COMPANY_NAME_TH"];
                model.Add(field);
            }
            ...
            if (req.IdentityType == UserTypeEnum.Citizen)
            {
                ...
                // ??? model.Add(new PDFFieldValue() { FieldName = "HomeNumber", Value = addrInfo[""] });
            ...
            model.Add(new PDFFieldValue() { FieldName = "Agent", Value = store.Data.TryGetString("INFORMATION_STORE_AGENT_TAX", "") });
        ...
        private static Queue<AppTaxEntry> GetAppTaxEntry(ApplicationRequestEntity req, string option)
        {
            ...
            var taxInfo = req.Data["APP_TAX_BOARD_INFO_SECTION"].Data;
            int count = int.Parse(taxInfo.TryGetString("APP_TAX_BOARD_INFO_SECTION_TOTAL", "0"));
