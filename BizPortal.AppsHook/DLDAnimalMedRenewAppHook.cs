...
{
    public class DLDAnimalMedRenewAppHook : StoreBaseAppHook
    {
        public override decimal? CalculateFee(List<ISectionData> sectionData)
        {
            decimal fee = 0;
            var sec = sectionData.Where(x => x.SectionName == "APP_ANIMAL_MED_RENEW_SECTION").FirstOrDefault();
            if (sec != null)
            {
                if (sec.FormData.TryGetString("APP_ANIMAL_MED_RENEW_TYPE_OPTION", "") == "APP_ANIMAL_MED_RENEW_TYPE_ONE")
                {
                    fee += 600;
                }
                if (sec.FormData.TryGetString("APP_ANIMAL_MED_RENEW_TYPE_OPTION", "") == "APP_ANIMAL_MED_RENEW_TYPE_TWO")
                {
                    var rest = int.Parse(sec.FormData["APP_ANIMAL_MED_RENEW_AMOUNT"].ToString());
                    if (rest < 11)
                    {
                        fee += 1000;
                    }
        ...
        public override JObject GenerateELicenseData(Guid applicationrequestId)
        {
            ...
            var organizationName = request.Data.TryGetData("INFORMATION_STORE").ThenGetStringData("INFORMATION_STORE_NAME_TH").ToThaiDigit();
            var organizationAmountOfBed = request.Data.TryGetData("APP_ANIMAL_MED_RENEW_SECTION").ThenGetStringData("APP_ANIMAL_MED_RENEW_AMOUNT").ToThaiDigit();
            ...
            if (request.Data.TryGetData("APP_ANIMAL_MED_RENEW_SECTION").ThenGetStringData("APP_ANIMAL_MED_RENEW_TYPE_OPTION") == "APP_ANIMAL_MED_TYPE_ONE")
            {
                organizationType = "ประเภทไม่มีที่พักสัตว์ป่วยไว้ค้างคืน";
                organizationAmountOfBed = "-";
            }
            else if (request.Data.TryGetData("APP_ANIMAL_MED_RENEW_SECTION").ThenGetStringData("APP_ANIMAL_MED_RENEW_TYPE_OPTION") == "APP_ANIMAL_MED_TYPE_TWO")
            {
